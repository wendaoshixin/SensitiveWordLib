package com.leimans.sensitivewordlib.sensitive_words.core;
...
public class SensitiveFilter implements Serializable{
	...
	public static SensitiveFilter createDefault(){
		/**
		 * 默认的单例，使用自带的敏感词库
		 */
		final SensitiveFilter DEFAULT = new SensitiveFilter(
	...
	public void setKeyWord(List<String> keyWordList){
		for (String keyWord:keyWordList){
			String real_key_word=keyWord.trim();
			if (real_key_word.length()>0){
				put(real_key_word);
			}
		}
	}
