...
import com.leimans.sensitivewordlib.toolgood.core.internals.BaseMatchEx;
...
public class WordsMatchEx extends BaseMatchEx {
    ...
    public com.leimans.sensitivewordlib.toolgood.core.WordsSearchResult FindFirst(final String text) {
        ...
        for (int i = 0; i < text.length(); i++) {
            ...
            if (p == 0 || t < _min[p] || t > _max[p]) {
                ...
                if (index == -1) {
                    if (_wildcard[p] > 0) {
                        final com.leimans.sensitivewordlib.toolgood.core.WordsSearchResult r = FindFirst(text, i + 1, _wildcard[p]);
                        if (r != null) {
                            return r;
                        }
                    }
            ...
            if (next != 0) {
                ...
                if (start < _end[next + 1]) {
                    ...
                    if (s >= 0) {
                        ...
                        return new com.leimans.sensitivewordlib.toolgood.core.WordsSearchResult(key, i + 1 - key.length(), i, index, matchKeyword);
                    }
                }
            }
            p = next;
    ...
    private com.leimans.sensitivewordlib.toolgood.core.WordsSearchResult FindFirst(final String text, final int index, int p) {
        for (int i = index; i < text.length(); i++) {
            ...
            if (start < _end[next + 1]) {
                ...
                if (s >= 0) {
                    ...
                    return new com.leimans.sensitivewordlib.toolgood.core.WordsSearchResult(key, i + 1 - key.length(), i, index2, matchKeyword);
                }
            }
            p = next;
        }
            ...
            if (next != 0) {
                for (int j = _end[next]; j < _end[next + 1]; j++) {
                    ...
                    if (s >= 0) {
                        ...
                        final String key = text.substring(s, i + 1);
                        final com.leimans.sensitivewordlib.toolgood.core.WordsSearchResult r = new com.leimans.sensitivewordlib.toolgood.core.WordsSearchResult(key, s, i, kIndex, matchKeyword);
                        result.add(r);
                    }
                }
            }
    ...
    private void FindAll(final String text, final int index, int p, final List<com.leimans.sensitivewordlib.toolgood.core.WordsSearchResult> result) {
        for (int i = index; i < text.length(); i++) {
            final char t1 = text.charAt(i);
            final int t = _dict[t1];
            if (t == 0) {
            ...
            for (int j = _end[next]; j < _end[next + 1]; j++) {
                ...
                if (s >= 0) {
                    ...
                    final String matchKeyword = _matchKeywords[kIndex];
                    final String key = text.substring(s, i + 1);
                    final com.leimans.sensitivewordlib.toolgood.core.WordsSearchResult r = new WordsSearchResult(key, s, i, kIndex, matchKeyword);
                    result.add(r);
                }
            }
            p = next;
